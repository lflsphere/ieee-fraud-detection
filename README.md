...
| `specification/scripts/` | One driver per phase; each writes to `reports/`. Kept out of version control (see `.gitignore`) along with the planning docs — the tracked repo holds the published deliverables. |
...
#    The phase drivers live in specification/scripts/, which is untracked —
#    see the layout table above.
PYTHONPATH=. python specification/scripts/01_dgp_evidence.py
PYTHONPATH=. python specification/scripts/02_build_eda_notebook.py
PYTHONPATH=. jupyter nbconvert --to notebook --execute --inplace \
    --ExecutePreprocessor.timeout=1800 notebooks/02_eda.ipynb
PYTHONPATH=. python specification/scripts/03_build_features.py
PYTHONPATH=. python specification/scripts/04_cluster_v_block.py
PYTHONPATH=. python specification/scripts/05_train_models.py
PYTHONPATH=. python specification/scripts/06_nn_training_dynamics.py
PYTHONPATH=. python specification/scripts/08_leakage_experiments.py

pytest src/tests -q
```

`03_build_features` and `04_cluster_v_block` write parquet caches to `data/interim/`
